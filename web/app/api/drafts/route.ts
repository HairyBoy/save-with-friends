...
export async function POST(req: Request) {
  ...
  const { owner, name, icon, goal, deadlineDays, payout } = (await req.json()) as {
    owner?: string;
    name?: string;
    icon?: string;
    goal?: string | number;
  ...
  const sql = getSql();
  await sql`
    insert into vault_drafts (id, owner_address, name, icon, goal, deadline_days, payout)
    values (${id}, ${owner.toLowerCase()}, ${name.trim().slice(0, 60)}, ${icon ?? "🏖️"}, ${String(goal)},
            ${Math.floor(deadlineDays)}, ${payout === 1 ? 1 : 0})`;
